...
pub fn EmployeeEdit(
    ...
    // フォーム用のローカルシグナルはここで作る（EmployeeItem の編集部分と同様）
    let mut employee_code = use_signal(|| employee.employee_code.clone().unwrap_or_default());
    let mut first_name = use_signal(|| employee.first_name.clone());
    let mut last_name = use_signal(|| employee.last_name.clone());
    let mut is_active = use_signal(|| employee.is_active);
            ...
            spawn(async move {
                ...
                .await
                {
                    Ok(_) => {
                        ...
                        // 編集完了したら右パネルを閉じる（親が on_close を受け取り editing_id を解除する想定）
                        on_close.call(());
                        on_refresh.call(());
                    }
                    Err(e) => {
    ...
    rsx! {
        div { class: "p-4",
            ...
            form { class: "grid gap-4 mb-4", onsubmit: move |e: Event<FormData>| {
                e.prevent_default();
                handle_save();
            },
