...
- ✅ ファイルアップロード機能
  ...
  - **クリック可能な大型ドロップエリアUI** - クリックでファイル選択ダイアログを開く
- ✅ ドキュメント一覧表示
  - カテゴリフィルター機能
  - ファイル情報表示（サイズ、アップロード日時）
  - アップロード成功時に自動更新
  - **画像プレビュー機能** - モーダルウィンドウで画像を大きく表示
  - **PDFプレビュー機能** - iframeでPDFを表示
- ✅ ダウンロード機能（S3 URLへの直接リンク）
- ✅ 削除機能（確認ダイアログ付き、S3とDBから同時削除）
- ✅ **アップロード進捗表示**
  - プログレスバー（0-100%）
  - 段階的ステータスメッセージ（準備中→アップロード中→完了）
...
#### 3.1 ドラッグ＆ドロップファイル処理
- 実際のドロップでファイルをアップロード（Dioxus将来バージョン対応）
- 複数ファイルの同時アップロード
